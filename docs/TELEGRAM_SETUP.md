...
> ⚠️ **הרצה ידנית באמצע היום — שימו לב לתיבת הסימון.** ל-*Run workflow* יש
> אפשרות **"Publish now, without waiting for 20:00 local time"**. היא **כבויה
> כברירת מחדל** בכוונה: אם מסמנים אותה, השיר של אותו יום נשלח **עכשיו**, ואז
> `--once-per-day` יחסום את ההרצה של 20:00 — כלומר בערב לא תגיע הודעה, כי
> היא כבר נשלחה. אם רק בודקים שהצינור עובד, אל תסמנו; אם באמת רוצים לפרסם
> את שיר היום מוקדם, סמנו.

> 🕐 **למה כל כך הרבה משבצות cron?** שתי סיבות. הראשונה: GitHub מריץ לפי UTC
> וישראל מזיזה שעון פעמיים בשנה. השנייה, והחשובה יותר: **GitHub לא מבטיח דיוק
> בתזמון** — הרצה שנקבעה לשעה עגולה מתחילה בפועל 20 עד 70 דקות מאוחר יותר, כי זו
> המשבצת העמוסה ביותר. לכן ה-workflow מבקש שעתי בדקה :23, ועוד ארבע משבצות ערב
> בדקה :07 — וכל הרצה שנוחתת ב-20:00 מקומי או אחריה מפרסמת. `--once-per-day`
> דואג שהשאר לא ישלחו שוב. התוצאה: הודעה אחת ביום, גם כשהתזמון מתעכב.

### ⚠️ המגבלה האמיתית: הרצות מתוזמנות הן אזרח סוג ב' אצל GitHub

זה לא כשל בקוד, וכדאי להכיר את זה. נמדד בריפו הזה ב-6/8/2026:

| סוג ההרצה | מה קרה |
|---|---|
| **Run workflow** (ידני) | קיבלה מכונה תוך שניות, הסתיימה תוך 3 דקות |
| **schedule** (cron) | ישבה **15 דקות בלי שהוקצתה לה מכונה בכלל** ואז בוטלה |

שלוש הרצות מתוזמנות רצופות מתו ככה באותו ערב, בזמן שהרצה ידנית באותה דקה עבדה
מצוין. GitHub נותנת עדיפות נמוכה ל-`schedule`, ובשעות עומס פשוט לא מקצה לו
מכונות. אין שום הגדרה בקובץ ה-workflow שמשנה את זה.

**מה עושים כשזה קורה?** לשונית **Actions** → *Daily Song* → **Run workflow**.
הרצה ידנית לא סובלת מהעדיפות הנמוכה, ותשלח את השיר תוך כשלוש דקות.

**פתרון קבוע, אם זה חוזר הרבה:** לתזמן מבחוץ במקום מבפנים. שירות cron חינמי
(למשל [cron-job.org](https://cron-job.org)) קורא פעם ביום ל-API של GitHub
ומפעיל **workflow_dispatch** — וזה, כאמור, מקבל מכונה מיד:

```
POST https://api.github.com/repos/sn1983/Shays-Music-Agent/actions/workflows/daily-song.yml/dispatches
Authorization: Bearer <PERSONAL_ACCESS_TOKEN עם הרשאת actions:write>
Content-Type: application/json

{"ref": "main"}
```

צריך ליצור טוקן אישי ב-GitHub (**Settings** → *Developer settings* →
*Fine-grained tokens*) עם הרשאת **Actions: Read and write** על הריפו הזה בלבד.
...
3. **אם השיר של היום כבר פורסם** (למשל הם נרשמו ב-21:30) — הוא נשלח אליהם
   מיד אחרי הודעת הפתיחה, כדי שלא יישארו עם מסך ריק עד מחר.
4. מאותו רגע הם מקבלים את השיר כל יום, יחד עם כולם.
5. `/stop` מסיר אותם, `/start` מחזיר.

> ההשלמה הזו עובדת רק על שירים שפורסמו אחרי העדכון שהוסיף אותה — הסוכן שומר
> מאז את המחקר המלא של כל שיר (`dossier_json`) כדי שיוכל לבנות מחדש את ההודעה.
> שיר ישן שנשמר בלי המידע הזה פשוט לא יישלח, בלי שגיאה.

### כמה זמן לוקח עד שהם מקבלים את הודעת הברוכים הבאים?

| איך מריצים | תגובה |
|---|---|
| GitHub Actions | **בפועל ~שעה ולפעמים יותר** — מבקשים כל 30 דקות, GitHub מחליטה מתי |
| `python main.py bot` על מחשב שדולק תמיד | מיידי |

**למה יש בכלל המתנה?** ל-GitHub Actions אין תהליך שרץ ברציפות — היא מתעוררת
לפי לוח זמנים, קוראת את ההודעות שהצטברו ומתה. טלגרם שומרת את ההודעות בתור עד
24 שעות, אז שום הרשמה לא הולכת לאיבוד; היא פשוט נקראת בהרצה הבאה.

`*/5` הוא המינימום ש-GitHub מאפשרת **לבקש**, אבל טריגרים מתוזמנים הם
best-effort ובריפו הזה נמדד משהו אחר לגמרי: ביום שבו ביקשנו `*/5` התקבלו 7
הרצות בכל היום, במרווחים של שעתיים עד ארבע — **ושלוש מארבע משבצות הערב נבלעו
בדרך**. לכן הבקשה ירדה ל-`*/30`: זו בקשה מציאותית שמשאירה ל-GitHub מקום
לספק דווקא את ההרצות שחשובות. וההבטחה למשתמש היא "תוך כשעה", לא "תוך כמה דקות".

כדי שהנרשם לא ירגיש שכלום לא קרה, מפרסמים לבוט תפריט פקודות ותיאור שמופיעים לו
**מיד** כשהוא פותח את הבוט — עוד לפני Start — ומסבירים מה יקבל ותוך כמה זמן
מגיע האישור. צריך להריץ את זה **פעם אחת בלבד**:

- **מ-GitHub, בלי להתקין כלום:** לשונית **Actions** → *Setup Bot Profile* →
  **Run workflow**.
- **מהמחשב:** `python main.py setup-bot`

> ⚠️ **שימו לב לדקות של Actions.** בריפו **ציבורי** ההרצות חינם ללא הגבלה.
> בריפו **פרטי** יש 2,000 דקות בחודש. `*/30` צורך בערך 1,450 בחודש — נכנס,
> אבל בקושי. אם תהפכו את הריפו לפרטי, שקלו `0 * * * *` (שעתי, ~730 דקות)
> או העבירו את ההאזנה ל-`python main.py bot` על מכונה שדולקת.
...
| הרצה ב-Actions מסומנת אדום, בלי אף שלב שרץ, `cancelled` אחרי 15 דקות | GitHub לא הקצתה מכונה להרצה מתוזמנת | לא באג בקוד. אם השיר לא יצא — **Run workflow** ידני. פירוט מלא בסעיף "המגבלה האמיתית" למעלה |
| מקבלים מיילים/התראות "workflow run failed" מ-GitHub | GitHub שולחת התראה על כל הרצה מתוזמנת שנכשלת, כולל כשלי תשתית | [github.com/settings/notifications](https://github.com/settings/notifications) → *Actions* → לבטל, או להשאיר רק על ריפו שעוקבים אחריו |
