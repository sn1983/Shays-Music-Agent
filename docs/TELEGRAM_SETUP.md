...
| איך מריצים | תגובה |
|---|---|
| GitHub Actions | **בפועל ~שעה** — מבקשים כל 5 דקות, אבל GitHub מריצה בערך פעם בשעה |
...
24 שעות, אז שום הרשמה לא הולכת לאיבוד; היא פשוט נקראת בהרצה הבאה. `*/5` הוא
המינימום ש-GitHub מאפשר לבקש — אבל טריגרים מתוזמנים הם best-effort, ובריפו הזה
נמדדה בפועל הרצה אחת בערך בכל שעה, גם כשביקשנו כל 5 או כל 15 דקות. לכן ההבטחה
למשתמש היא "תוך כשעה" ולא "תוך כמה דקות".
...
> בריפו **פרטי** יש 2,000 דקות בחודש, ו-`*/5` צורך בערך 8,600 — הרבה מעבר.
> אם תהפכו את הריפו לפרטי, העלו את ה-cron ל-`*/30` לפחות, או העבירו את
> ההאזנה ל-`python main.py bot` על מכונה שדולקת.
